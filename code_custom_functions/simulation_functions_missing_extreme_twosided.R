###########################
#   CUSTOM FUNCTIONS FOR SIMULATIONS (GENERATING DATA / POWER ANALYSIS): THE MOST EXTREME / LOWEST + HIGHEST (1-C)/2 DATA MISSING
###########################
...
AR_simulate_data_extreme_twosided = function(N,T.obs,Ylag.center,
  ...
  for (i in n.ID){
    ...
    for (t in T.obs.i[-1]){
      ...
      compliance[t] = compliance[i] # save the individual participant's compliance 
  ...
  # Introduce missing data in the time-series, separately for each participant
  data = data.frame()
  for(participant_id in 1:N){
    data_participant = data.Y[data.Y$subjno == participant_id,] # filter the data for each participant in N 
    compliance = data_participant[1, 4] # extract the individual participant's compliance
    true_mean = data_participant[1, 5]
    threshold_quantiles = quantile(data_participant$Y, c((1-compliance)/2, 1-((1-compliance)/2))) # find the value of the quantiles below and above which we will remove values
    data_participant$Y[data_participant$Y < threshold_quantiles[1]] = NA # set all values below the lower threshold quantile to NA
    data_participant$Y[data_participant$Y > threshold_quantiles[2]] = NA # set all values above the lower threshold quantile to NA
...
AR_fit_model_extreme_twosided = function(data,N,T.obs,Ylag.center,
  ...
  if (length(fit.lme)>1){
    ...
    # Obtain the number of observation used for estimation
