###########################
#   CUSTOM FUNCTIONS FOR SIMULATIONS (GENERATING DATA / POWER ANALYSIS): DATA MCAR 
###########################
...
# 1) AR_simulate_data_mcar = Data generation function. Creates a single dataset, introduces data missing at random
...
AR_simulate_data_mcar = function(N,T.obs,Ylag.center,
  ...
  # Introduce MCAR data in the time-series, separately for each participant
  data = data.frame()
  for(participant_id in 1:N){
    ...
    data_participant_mcar = as.data.frame(delete_MCAR(ds = data_participant, # introduce data MCAR
                                                      cols_mis = 3,
                                                      p = (1 - compliance)))
    data = rbind(data, data_participant_mcar)
  }
...
AR_fit_model_mcar = function(data,N,T.obs,Ylag.center,
...
###########################################
###################### AR_simulate_missing_mcar
###########################################

AR_simulate_missing_mcar = function(N,T.obs,Ylag.center,
