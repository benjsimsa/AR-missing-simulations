###########################
#   CUSTOM FUNCTIONS FOR SIMULATIONS (GENERATING DATA / POWER ANALYSIS): THE MOST EXTREME / LOWEST (1-C) DATA MISSING
###########################
...
Sim.AR.Model.extreme_oneside = function(N,T.obs,Ylag.center,
  ...
  for(participant_id in 1:N){
    ...
    threshold_quantile = quantile(data_participant$Y, 1-compliance) # find out the value of the quantile below which we will remove values
    data_participant$Y[data_participant$Y < threshold_quantile] = NA # set all values below the threshold quantile to NA
