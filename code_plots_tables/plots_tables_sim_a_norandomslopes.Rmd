...
```{r table_all}
overall_df_reordered = overall_df[, c(11, 14, 10, 8, 9, 2, 4, 1, 3, 6, 5, 7, 12, 13, 15)] # reordering the dataframe so that the order of columns makes more sense 
...
# uncomment the following line to export the table to docx 
tab_df(overall_missing_type, file = here::here("plots_tables", "a_overall_missing_type.doc"))
```

Interesting things here: - for all conditions except for MCAR, the bias gets worse with increasing real fixed AR - bias is smallest for mcar, worst for twosided extreme
...
```{r }
(conv_avg = ggplot(data = overall_by_NT,
                  mapping = aes(x = comp_mean,
                                y = conv)) + 
                  geom_line(aes(colour = as.factor(miss_type),
                                linetype = as.factor(miss_type))) +
                  ...
                  theme_minimal() +
                  scale_x_continuous(breaks = c(0.4, 0.6, 0.8, 1)) + 
                  ylab("Proportion of converged models") + 
                  xlab("Mean compliance") + 
                  labs(colour = "Missingness type",
                       linetype = "Missingness type") +
                  facet_grid(~ B1_sim)
...
conv_avg = ggplotly(conv_avg)
```

### Number of effective observation-pairs

```{r }


(obs_avg = ggplot(data = overall_by_NT,
                  mapping = aes(x = comp_mean,
                                y = model_observations)) + 
                  geom_line(aes(colour = as.factor(miss_type),
                                linetype = as.factor(miss_type))) +
                  geom_point() + 
                  theme_minimal() +
                  scale_x_continuous(breaks = c(0.4, 0.6, 0.8, 1)) + 
                  ylab("Number of effective observation-pairs") + 
                  xlab("Mean compliance") + 
                  labs(colour = "Missingness type",
                       linetype = "Missingness type") +
                  facet_grid(~ B1_sim)
...
obs_avg = ggplotly(obs_avg)
```

### Bias in the estimation of individual means

```{r }
(bias_mean_avg = ggplot(data = overall_by_NT,
                  mapping = aes(x = comp_mean,
                                y = bias_mean)) + 
                  geom_line(aes(colour = as.factor(miss_type),
                                linetype = as.factor(miss_type))) +
                  ...
                  theme_minimal() +
                  scale_x_continuous(breaks = c(0.4, 0.6, 0.8, 1)) + 
                  ylab("Bias in estimation of individual means") + 
                  xlab("Mean compliance") + 
                  labs(colour = "Missingness type",
                       linetype = "Missingness type") +
                  facet_grid(~ B1_sim)
...
bias_mean_avg = ggplotly(bias_mean_avg)
```

## Visualising power

```{r viz pow function}
viz_NT_pow = function(N_select, T.obs_select, data = overall_df, B10 = 0.3){
  # Get subset of the data that has the particulaR N/T combination 
  d = data %>% 
   filter(data$N == N_select & data$T.obs == T.obs_select & data$B1_sim == B10)
  # Visualise the data
  pow_avg = ggplot( data = d,
                  ...
                  theme_minimal() +
                  scale_x_continuous(breaks = c(0.4, 0.6, 0.8, 1)) +
                  ylim(0.1, 1) + #setting the y-limits manually          
...
for (n in 1:length(Ns)){
  for (t in 1:length(Ts)){
    plot = viz_NT_pow(Ns[n], Ts[t], B1 = 0.3)
    plotlist_03 = c(plotlist_03, list(plot))
  }
}
...
viz_NT_est = function(N_select, T.obs_select, data = overall_df, B10 = 0.3){
  # Get subset of the data that has the particulaR N/T combination 
  d = data %>% 
   filter(data$N == N_select & data$T.obs == T.obs_select & data$B1_sim == B10)
  # Visualise the data
  plot = ggplot(data = d,
                  ...
                  theme_minimal() +
                  scale_x_continuous(breaks = c(0.4, 0.6, 0.8, 1)) +
                  ylim(-0.1, 0.4) + #setting the y-limits manually          
    theme(legend.position = "none") +
    theme(axis.title = element_blank())
    return(plot)
}
...
plotlist = list(
  plot1 = viz_NT_est(20, 20),
  plot2 = viz_NT_est(20 ,50),
  plot3 = viz_NT_est(20, 100),
  plot4 = viz_NT_est(100, 20),
  plot5 = viz_NT_est(100 ,50),
  plot6 = viz_NT_est(100, 100)
)
...
### B1 *and* B0 for all N/T combinations
...
# ANOVA for estimation performance

Get the 1000 estimated fixed effects from each condition
...
Run an ANOVA on the big dataset

TODO: get omega-squared

```{r}
anova = aov(B1_bias ~ B1_sim * N * T.obs * miss_type * compliance, data = anova_df)
