...
```{r }
(se_avg_sd = ggplot(data = overall_by_NT,
...
### Power 
```{r }
(pow_avg_sd = ggplot(data = overall_by_NT,
                  mapping = aes(x = comp_mean,
                                y = B1_pow)) + 
                  geom_line(aes(colour = as.factor(miss_type),
                                linetype = as.factor(miss_type))) +
...
pow_avg_sd = ggplotly(pow_avg_sd)
...
### Fixed AR estimate 
```{r }
(b1_avg_sd = ggplot(data = mcar_df_NT,
                  mapping = aes(x = comp_mean,
                                y = B1_est, 
                                group = comp_sd)) + 
                  ...
                  geom_hline(yintercept = 0.20, 
                        linetype = "dashed") + 
                  ggtitle("Real fixed AR(1) effect = 0.2")
)

b1_avg_sd = ggplotly(b1_avg_sd)
```
