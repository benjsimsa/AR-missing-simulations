...
Furthermore, the interaction between the number of time points per participant and compliance ($\omega^{2}_p$ = `r round(anova_table_se[10, 6], 2)`), number of participants and time points per participant ($\omega^{2}_p$ = `r round(anova_table_se[6, 6], 2)`), and between the number of participants and compliance ($\omega^{2}_p$ = `r round(anova_table_se[9, 6], 2)`) was found.
...
Figure \ref{fig:fig_se_N_sim2} illustrates the main effect of *N* (as the most important factor) on standard error, while Figure \ref{fig:fig_se_compliance_T_sim2} shows the interaction between compliance and *T.obs*. While the results are comparable to Simulation A, the SE is slightly higher for the same combinations of number of participants / timepoints per participant in Simulation B.
