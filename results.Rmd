...
```{r fig_suppl_bias_type, echo=FALSE, fig.cap = "\\label{fig:fig_suppl_bias_type}Supplementary Simulation C (no person-mean centering): The effect of missingnness type on estimation bias", message=FALSE, warning=FALSE}
...
omegasq_bias_simD = data.frame(omega_squared(anova_bias_simD, partial = TRUE)) %>% 
  select(Omega2_partial) %>%
  rename("Partial omega-squared" = "Omega2_partial")
omegasq_bias_simD[17, ] = NA # add omega-squared of 0 for residuals
...
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
table_bias_simD_ = anova_bias_df_simD %>% 
  group_by(miss_type, compliance) %>% 
  summarise(round(mean(B1_bias),2)) %>% 
  rename("bias" = "round(mean(B1_bias), 2)") %>% 
  dcast(compliance ~ miss_type, value.var = "bias")
...
```{r table_bias_simD_average, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}


kable(table_bias_simD_, 
      format = "latex", 
      booktabs = TRUE,  
      digits = 2,
