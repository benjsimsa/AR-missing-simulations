...
The descriptive results for all 288 conditions in Simulation A are reported in the appendix (TODO).
...
**ANOVA.** We used a 4 × 2 × 3 × 4 × 3 factorial Type I ANOVA (with estimation bias as an outcome and number of participants, number of timepoints per participant, missingness type, compliance, and the simulated fixed autoregressive effect) to assess which of the manipulated factors had a considerable influence on estimation bias. The results from every simulation run (i. e., 1,000 results per condition = 288,000 rows) were combined into a single dataset for the analysis. Given the very large sample size (which would make any difference significant) and the exploratory character of the analysis, *p*-values and significance thresholds were not used make inferences. Instead, we used a threshold of 0.14 for the partial $\omega^2$, indicating a large effect size [@field2012discovering]. This cutoff will be used for all ANOVA results throughout the results section. The partial $\omega^2$ was chosen as the less biased alternative to partial $\eta^2$ [@okada2013omega]. The results and effect sizes are reported in Table FJ.

Four main effects above the effect size threshold of 0.14 were found: the main effect of missingness type ($\omega^{2}$ = `r round(anova_table[3, 6], 2)`), compliance ($\omega^{2}$ = `r round(anova_table[4, 6], 2)`), the number of timepoints per participant ($\omega^{2}$ = `r round(anova_table[2, 6], 2)`), and the simulated fixed slope ($\omega^{2}$ = `r round(anova_table[5, 6], 2)`). Furthermore, the interaction between the missingness type and compliance ($\omega^{2}$ = `r round(anova_table[11, 6], 2)`) had an effect size above the cut-off.

The main effects of missingness type and compliance are visualised in Figure BG and Figure BH (respectively), while the interaction between missingness type and compliance are depicted in Figure BK.

```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap = "The effect of compliance on the bias in estimation of the fixed slopes.", fig.width=6.4, fig.height=3.5}
...
```{r  echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap = "The effect of the interaction between missingness type and compliance on the bias in estimation of the fixed slopes.", fig.width=6.4, fig.height=3.5}
...
The main effects of the number of participants ($\omega^{2}$ = `r round(anova_table_se[1, 6], 2)`), number of timepoints per participant ($\omega^{2}$ = `r round(anova_table_se[2, 6], 2)`) and compliance ($\omega^{2}$ = `r round(anova_table_se[4, 6], 2)`) crossed the cut-off for effect size.

Additionally, the interaction between the number of timepoints per participant and compliance ($\omega^{2}$ = `r round(anova_table_se[10, 6], 2)`), number of participants and timepoints per participants ($\omega^{2}$ = `r round(anova_table_se[5, 6], 2)`), and between the number of participants and compliance ($\omega^{2}$ = `r round(anova_table_se[9, 6], 2)`) was found.

The average standard errors for the different combinations of number of participants, timepoints per participant and compliance are reported in Table XC. Figure @ref(fig:fig_se_interaction) depicts the interaction between the number of timepoints per participant and compliance, while Figure \ref{fig:fig_se_compliance} shows the interaction between the number of participants and compliance.
...
```{r fig_se_interaction, echo=FALSE, fig.height=3.5, fig.width=6.4, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap = "\\label{fig:fig_se_interaction}The effect of the interaction between number of timepoints and compliance on standard error of estimation of the fixed slopes. Simulation A."}
# Figure AD 
...
```{r fig_se_compliance, echo=FALSE, fig.height=3.5, fig.width=6.4, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap = "\\label{fig:fig_se_compliance}The effect of the interaction between number of participants and compliance on standard error of estimation of the fixed slopes. Simulation A."}
# Figure AF 
...
**Descriptive statistics.** See Figure \@ref(fig:fig_power)
