...
In Simulation B, the variance of random AR effects ($\sigma_{\nu1}$) was manipulated as an additional simulation factor (2 values: 0.05 and 0.1). For an overview of all manipulated and fixed simulation parameters, please refer to Table \\ref{tab:tab_manipulated}. The descriptive results for all 256 simulation conditions are reported in the appendix (TODO). For the sake of clarity, in the following section, only the results for $\sigma_{\nu1}$ will be reported (and then briefly compared to the results of the $\sigma_{\nu1}$ = 0.1 condition).
